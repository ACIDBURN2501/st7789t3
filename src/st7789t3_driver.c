 ...
 */
#include "../include/st7789t3_driver.h"
...
int
lcd_init(int spi_fd, int gpio_dc, int gpio_rst)
{
...
int
lcd_show_splash(const char *path)
{
        int res = image_load_rgb565(path, lcd_buf, LCD_W, LCD_H);
        if (res == 0) {
                st7789t3_draw_buffer_area(0, 0, LCD_W, LCD_H, lcd_buf);
        }
        return res;
}

void
lcd_show_text(const char *msg, uint16_t fg, uint16_t bg)
{
        st7789t3_fill_rect(0, 0, LCD_W, LCD_H, bg);
